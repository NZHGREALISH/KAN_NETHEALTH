...
def load_and_merge_data(fitbit_file, scales_file):
    print("正在加载数据...")
    fitbit_df = pd.read_csv(fitbit_file)
    print(f"Fitbit数据: {len(fitbit_df)} 行")
    scales_df = pd.read_csv(scales_file)
    print(f"量表数据: {len(scales_df)} 行")
    fitbit_processed = preprocess_fitbit_data(fitbit_df)
    merged_df = pd.merge(fitbit_processed, scales_df, on='egoid', how='inner')
...
def prepare_features_targets(merged_df):
    ...
    y = merged_df[[available_targets[task] for task in target_names]].copy()
    y.columns = target_names
    processed = pd.concat([merged_df[['egoid']], X, y], axis=1)
    return processed

def main():
    fitbit_file = "/home/grealish/summer/KAN_NET/check/FitbitActivity(1-30-20).csv"
    scales_file = "/home/grealish/summer/KAN_NET/predict/extracted_scales_data.csv"
    output_file = "/home/grealish/summer/KAN_NET/predict/processed_data.csv"
    try:
        merged_df = load_and_merge_data(fitbit_file, scales_file)
        if len(merged_df) == 0:
            print("错误: 合并后的数据为空")
            return
        processed = prepare_features_targets(merged_df)
